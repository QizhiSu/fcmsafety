...
| 加一个新法规库 | ① `inst/fcmsafety_schema.sql` 加表 ② `download_sources.R` 加抓取 ③ `update_other_dbs.R` 加 `update_*_auto()` ④ `incremental_update.R` 的 `db_col_candidates` 登记列名差异 ⑤ `ALL_AUTO_DBS` 加名字 | 五处都要动，漏一处会静默不生效 |
