...
fcmsafety::update_cmr_auto(enrich = TRUE)    # CMR from CLP Annex VI
fcmsafety::update_iarc_auto()                # IARC classifications
...
result <- run_toxtree(data)
# Other Cramer rule variants:
# run_toxtree(data, module = "cramer2.CramerRulesWithExtensions")               # extended
# run_toxtree(data, module = "toxtree.tree.cramer3.RevisedCramerDecisionTree")  # revised
```

## Manual Toxtree batch processing (alternative)

If you prefer the Toxtree GUI, or cannot install Java, the classical
manual workflow is still available: import `for_toxtree.csv` into the
Toxtree application, run batch processing with the Cramer rules module,
and save the result as `toxtree_results.csv`.
